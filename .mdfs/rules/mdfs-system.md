...
## When this prompt is embedded in a user document (no system prompt available)

If you are reading these rules as part of a user-supplied document (not as a
system prompt), follow these additional instructions:

1. **Do NOT output the rules themselves** in your final answer. After reading
   and understanding them, proceed directly to processing the document content
   that follows.

2. **Confirmation protocol**:  
   - If you fully understand the rules and can apply them, simply continue
     with the task (process the files/patches as described).  
   - If any rule is unclear or you need clarification, respond with
     `"I need clarification on: <specific rule>"` and do not attempt to
     process the document until you receive clarification.

3. **Assume the document after these rules contains project files and patches**.
   Apply all the formatting requirements (markers, fence depth, patch context,
   etc.) when you generate or modify files.
